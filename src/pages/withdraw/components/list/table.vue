<template>
    <div class="container">
        <a-space class="container-action" size="large">
            <a-button v-action="'/withdraw/remove'" :disabled="selectedRowKeys.length > 0 ? false :true" @click="batchRemove" type="danger">
                批量删除
            </a-button>
        </a-space>
        <a-table 
                ...
                @change="pageChange"
            >
            <span slot="withdrawMoney" slot-scope="withdrawMoney">
                {{withdrawMoney}} ￥
            </span>
            ...
            <span slot="action" slot-scope="text, record">
                <a-space>
                    ...
                    >
                        <a v-action="'/withdraw/review'" v-if="record.status == 1">审核</a>
                    </a-popconfirm>
                    <a v-action="'/withdraw/remove'" @click="remove(record.id)">删除</a>  
                </a-space>
...
<script>
import { Remove,Review } from '@/api/withdraw'
export default {
    props:{
        list:{
            type:Array,
